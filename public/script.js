...
// Função para validar token de redefinição
async function validateResetToken(token) {
    try {
        ...
        if (response.ok) {
            ...
            showError(loginErrorMessage, data.error || "Token inválido ou expirado");
            loginScreen.classList.add("active");
        }
    } catch (error) {
        console.error("Erro ao validar token:", error);
        showError(loginErrorMessage, "Erro ao validar token");
        loginScreen.classList.add("active");
    }
}
...
registerForm.addEventListener("submit", async (e) => {
  ...
  try {
    ...
    // Salvar dados no localStorage (simulação de login)
    const userData = {
      username: username,
      email: email,
      userId: data.userId,
...
loginForm.addEventListener("submit", async (e) => {
  ...
  try {
    // Simulação de login: verificar se o usuário existe no localStorage
    // Em um projeto real, esta verificação seria feita no backend
    const storedUser = JSON.parse(localStorage.getItem("cardYXSUser"));

    if (
      storedUser &&
      (storedUser.email === loginIdentifier || storedUser.username === loginIdentifier) &&
