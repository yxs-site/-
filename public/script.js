...
(function () {
  ...
  function resetGame() {
    ...
    showGameSection(tictactoeLobby);
  }

  // 1. Abrir Modal
  safe(btnOpenTictactoe, el => el.addEventListener('click', () => {
    if (modalTictactoe) modalTictactoe.classList.add('active');
  ...
  safe(btnLeaveGame, el => el.addEventListener('click', () => {
    socket.emit('leave-room');
    if (modalTictactoe) modalTictactoe.classList.remove('active');
    resetGame();
  }));
  ...
  socket.on('player-joined', (data) => {
    // A lógica de playerSockets.get(socket.id) é de backend e não funciona no frontend.
    // O roomCode já deve estar definido no frontend se o jogador criou ou tentou entrar na sala.
    // Se o jogador está entrando, o roomCode é o que ele digitou.
    // Se o jogador criou, o roomCode foi definido no evento 'room-created'.
    // Apenas garantimos que a mensagem de 'Tentando entrar...' seja limpa.
    lobbyMessage.textContent = ''; 
  ...
  socket.on('player-left', (data) => {
    gameStatus.textContent = `${data.playerName} saiu da sala.`;
    gameInfo.textContent = 'O jogo terminou.';
    btnPlayAgain.style.display = 'none';
    setTimeout(() => {
      if (modalTictactoe && modalTictactoe.classList.contains('active')) {
        modalTictactoe.classList.remove('active');
        resetGame();
      }
    }, 3000);
  });
