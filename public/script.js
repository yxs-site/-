...
(function () {
  ...
  async function removeProfilePicture() {
    ...
    if (!storedUser || !storedUser.token) {
      console.warn("Usuário não autenticado. Não é possível remover a foto.");
      return;
    }
    ...
    try {
      ...
      if (!res.ok) {
        const d = await res.json().catch(() => ({}));
        throw new Error(d.error || "Erro ao remover foto de perfil");
      }
      ...
      console.error("Erro ao remover foto:", err.message || err);
    }
  }
